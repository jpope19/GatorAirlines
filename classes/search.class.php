...
class Search {
  ...
  private function set_routes() {
    ...
    $depart_routes = $this->find_routes($this->opts['org'], $this->opts['dest'], $flights);
    $flights = $this->user->get_flights($this->opts['e_return_time']);
    $return_routes = $this->find_routes($this->opts['org'], $this->opts['dest'], $flights);
  }
  ...
  private function find_routes($org, $dest, &$flights) {
    ...
    //BFS to find all routes that take < 48 hours
    while($queue->count() >0)
    {
      $cur_route = $queue.dequeue();
      foreach($flights as $flight) {
        if($flight['org_id'] == $cur_route.get_dest()) {
          $new_route = $cur_route.copy();
          $new_route.add($flight);
          if($new_route.get_trip_time() < 48*60*60) {
            if($new_route.get_dest() == $opts['dest']) {
              $result[] = $new_route;
            } else {
